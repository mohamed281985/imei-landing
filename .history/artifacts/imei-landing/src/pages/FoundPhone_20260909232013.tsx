...
interface FoundResponse {
  ...
  owner_phone?: string;
  owner_contact_available?: boolean;
  anther_number?: string;
  code?: string;
}
...
export default function FoundPhone() {
  ...
  const [showOwnerContactOptions, setShowOwnerContactOptions] = useState(false);
  const [finderPhone, setFinderPhone] = useState("");
  const [captchaToken, setCaptchaToken] = useState("");
  const captchaContainerRef = useRef<HTMLDivElement>(null);
  const captchaWidgetIdRef = useRef<string | null>(null);
  const { toast } = useToast();
  const ownerNumber = data?.owner_phone?.trim() || data?.anther_number?.trim() || "";

  useEffect(() => {
    if (!data || ownerNumber) return;
  ...
  const ownerPhoneHref = useMemo(() => {
    const normalizedOwnerNumber = ownerNumber.replace(/[^0-9+]/g, "");
    return normalizedOwnerNumber ? `tel:${normalizedOwnerNumber}` : "";
  }, [ownerNumber]);

  const ownerWhatsappHref = useMemo(() => {
    const normalizedOwnerNumber = ownerNumber.replace(/[^0-9]/g, "");
    return normalizedOwnerNumber ? `https://wa.me/${normalizedOwnerNumber}` : "";
  }, [ownerNumber]);

  const handleContactOwner = async () => {
    if (!token || contactStatus === "sending") return;

    const normalizedFinderPhone = finderPhone.replace(/[^0-9+]/g, "");
    if (normalizedFinderPhone.replace(/\D/g, "").length < 7) {
      toast({
        title: "رقم الهاتف مطلوب",
        description: "اكتب رقم هاتفك حتى يتمكن المالك من التواصل معك.",
        variant: "destructive",
      });
      return;
    }
                    ...
                    {ownerNumber && (
                      ...
                      >
                        <div className="flex flex-col items-stretch gap-3">
                          <Button
                            type="button"
                            onClick={() => setShowOwnerContactOptions((isOpen) => !isOpen)}
                            className="bg-blue-600 hover:bg-blue-700 text-white rounded-2xl px-8 py-3 font-semibold shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
                          >
                            <Phone className="h-5 w-5" />
                            اتصل بالمالك
                          </Button>
                          <AnimatePresence>
                            {showOwnerContactOptions && (
                              <motion.div
                              ...
                              >
                                <Button
                                  asChild
                                  className="bg-blue-500 hover:bg-blue-600 text-white rounded-2xl px-6 py-3 font-semibold flex items-center justify-center gap-2"
                                >
                                  <a href={ownerPhoneHref}>
                                    <Phone className="h-5 w-5" />
                                    اتصال هاتفي
                                  </a>
                                </Button>
                                <Button
                                  asChild
                                  className="bg-green-600 hover:bg-green-700 text-white rounded-2xl px-6 py-3 font-semibold flex items-center justify-center gap-2"
                                >
                                  <a href={ownerWhatsappHref} target="_blank" rel="noreferrer noopener">
                      ...
                      >
                        <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center">
                          ...
                          <Button
                            ...
                            disabled={contactStatus !== "idle"}
                            className="bg-amber-500 hover:bg-amber-600 text-white rounded-2xl px-8 py-3 font-semibold shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-70"
                          >
