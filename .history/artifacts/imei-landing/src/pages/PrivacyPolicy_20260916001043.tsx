...
import { ArrowLeft, ShieldCheck, Lock, Smartphone, UserCog, RefreshCcw, Mail, CheckCircle2, Menu, X, Play } from "lucide-react";
...
const translations = {
  ar: {
    ...
    accountDeletionTitle: "طلب حذف الحساب والبيانات",
    accountDeletionText:
      "يمكنك طلب حذف حسابك وجميع بياناتك وأجهزتك المسجلة في أي وقت. لإتمام الطلب، أرسل رسالة من البريد الإلكتروني المرتبط بحسابك إلى:",
    accountDeletionSubject: "طلب حذف حساب IMEI Safe",
    accountDeletionButton: "إرسال طلب حذف الحساب",
  ...
  en: {
    ...
    accountDeletionText:
      "You may request deletion of your account, all associated data, and registered devices at any time. To submit a request, email us from the address linked to your account at:",
    accountDeletionSubject: "IMEI Safe account deletion request",
    accountDeletionButton: "Email an account deletion request",
    footer: "Back to Home",
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex h-20 items-center justify-between md:h-24">
            <a href="/#hero" className="flex items-center gap-3">
              ...
              />
              <span className="hidden text-xl font-bold text-sky-700 sm:block md:text-2xl">تطبيق IMEI</span>
            </a>

            <nav className="hidden items-center gap-6 text-sm font-medium text-slate-700 lg:flex">
              <a href="/#hero" className="transition-colors hover:text-sky-700">الرئيسية</a>
              <a href="/#features" className="transition-colors hover:text-sky-700">المميزات</a>
              <a href="/#pricing" className="transition-colors hover:text-sky-700">الباقات</a>
              <a href="/#faq" className="transition-colors hover:text-sky-700">الأسئلة الشائعة</a>
              <Link href="/terms-and-conditions"><a className="transition-colors hover:text-sky-700">شروط الاستخدام</a></Link>
              <Link href="/privacy-policy"><a className="font-bold text-sky-700">سياسة الخصوصية</a></Link>
            </nav>
            ...
            <div className="flex items-center gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="hidden items-center rounded-full bg-sky-600 px-5 py-2.5 text-base font-semibold text-white shadow-lg shadow-sky-600/20 transition hover:bg-sky-500 sm:flex"
              >
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-white/95 px-4 pt-24 backdrop-blur-sm lg:hidden">
          <nav className="flex flex-col gap-6 text-center text-lg font-medium text-slate-800">
            ...
            <a href="https://play.google.com/store/apps/details?id=com.imei.app" target="_blank" rel="noreferrer" className="inline-flex items-center justify-center rounded-full bg-sky-600 py-4 text-base font-bold text-white">
              <Play className="ml-2 h-5 w-5" />
              حمّل من Google Play
            </a>
          </nav>
        ...
        <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col items-center px-4 py-8 lg:px-6">
          ...
          >
            ...
            <motion.section
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="mt-8 rounded-3xl border border-rose-200 bg-linear-to-br from-rose-50 via-white to-orange-50 p-5 shadow-md sm:p-6"
            >
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h2 className="mb-3 text-xl font-black text-slate-900 sm:text-2xl">{content.accountDeletionTitle}</h2>
                  <p className="max-w-3xl text-base leading-8 text-slate-700">{content.accountDeletionText}</p>
                  <a href="mailto:imeisafe@gmail.com" className="mt-2 inline-block text-lg font-black text-rose-700 underline decoration-rose-300 underline-offset-4">
                    imeisafe@gmail.com
                  </a>
                  <p className="mt-3 text-sm leading-7 text-slate-600">
                    {isArabic ? "يفضل كتابة عنوان الرسالة:" : "Please use the subject:"}{" "}
                    <strong>{content.accountDeletionSubject}</strong>
                  </p>
                </div>
                <a
                  href={`mailto:imeisafe@gmail.com?subject=${encodeURIComponent(content.accountDeletionSubject)}`}
                  className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-rose-600 px-5 py-3 text-center text-sm font-bold text-white shadow-lg shadow-rose-600/20 transition hover:bg-rose-500"
                >
                  <Mail className="h-4 w-4" />
                  {content.accountDeletionButton}
                </a>
              </div>
            </motion.section>
